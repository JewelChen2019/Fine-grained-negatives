...
class XCLIP(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, input_ids_aug, token_type_ids_aug, attention_mask_aug,video_mask=None):
    # def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        neg_sequence_output = []
        neg_seq_features =[]
        for tep_input_ids_aug,tep_token_type_ids_aug,tep_attention_mask_aug in zip( input_ids_aug, token_type_ids_aug, attention_mask_aug):
        ...
        if self.training:
            if do_boundary:
                ...
                # import pdb;pdb.set_trace()
                #### negtive sample data augmentation
                # import pdb;pdb.set_trace()
                '''
                #triplet loss
                ...
                # v2negt_sim_matrix=[]
                # for singel_video_feat,singel_video_mask,k_neg_sent_feat,k_neg_seq_features,k_neg_attention_mask in zip(visual_output,video_mask,neg_sequence_output,neg_seq_features,attention_mask_aug):
                ...
                # concatenated_tensor_semantic = torch.cat((sim_matrix.T, v2negt_sim_matrix[:, 1:]), dim=1)
                # sim_loss_neg = self.loss_fct(concatenated_tensor_semantic)#将负样本置于分母 ？？ 也许分开 会好一些？单独的去算而不是和
                 
                # sim_loss_boundary_t2t,sim_loss_boundary_t2v, = self.loss_fct_boundary(sequence_output,visual_output,sequence_output_neg)##之前改的loss_boundary 有点问题，现在改为直接输入相似度矩阵
                
                #### 计算t2v nag2v t2neg 的相似度
                # if do_triplet ==True:
                neg_sim_matrix, *_tmp = self.get_similarity_logits(sequence_output_neg, seq_features_neg, visual_output, attention_mask,  video_mask, shaped=True, loose_type=self.loose_type)
                t2neg_sim_matrix =self.get_t2t_similarity_logits(sequence_output_neg, sequence_output, seq_features_neg,seq_features)
                # sim_loss_neg = self.loss_fct_triplet(sim_matrix,neg_sim_matrix)
                # import pdb;pdb.set_trace()
                sim_loss_boundary_t2t,sim_loss_boundary_t2v, = self.loss_fct_boundary(sim_matrix,neg_sim_matrix,t2neg_sim_matrix)#input 为t2v_sim_matrix, neg2v_sim_matrix, t2neg_sim_matrix
    
                
                #只计算 boundary loss
                sim_loss = (sim_loss1 + sim_loss2)/2 + 0.02*(sim_loss_boundary_t2t+ sim_loss_boundary_t2v )
                ...
                sim_loss_neg = self.loss_fct_hitriplet(sim_matrix,v2negt_sim_matrix[:, 1:])## wrong!!!!!!
                sim_loss = (sim_loss1 + sim_loss2 ) / 2 + 0.2*sim_loss_neg
                loss += sim_loss
            else:
                loss = 0.
