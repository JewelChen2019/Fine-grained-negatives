...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.vpt_embedding = nn.Parameter(torch.zeros( 5, width),requires_grad = True)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_hidden=False, return_vp=False, video_frame=-1):
        ...
        # import pdb; pdb.set_trace()
        x = hidden[:, 0, :]  # extract the [CLS] token
        vp_embedding = hidden[:, 1:6, :] 
        mean_pooled_vp = torch.mean(vp_embedding, dim=1, keepdim=True).squeeze(1)
        ...
        if return_vp:
            ...
            vp_embedding = hidden[:, 5, :] 
            return x, vp_embedding

        # return x
        print("----------------- mean_pooled_vp is working -----------------")
