...
class Margin2Loss(nn.Module):##修改
    """
    Compute margin  loss
    arg input: (batchsize, subspace) and (batchsize, subspace)
    """

    def __init__(self, bottommargin,uppermargin, bottommargin_t2t,uppermargin_t2t, measure='cosine', cost_style='sum',
    ...
    def forward(self, t2v_sim_matrix, neg2v_sim_matrix, t2neg_sim_matrix):
      
        # compute image-sentence score matrix
        # scorest = self.sim(txt_embs, vis_embs.mean(dim=1, keepdim=True))
        # scoresf = self.sim(false_txt_embs, vis_embs.mean(dim=1, keepdim=True))
        # scoresf2 = self.sim(false_txt_embs, txt_embs)

        scorest=t2v_sim_matrix
        scoresf=neg2v_sim_matrix
        scoresf2=t2neg_sim_matrix
        ...
        if self.uppermargin_t2t is not None or self.bottommargin_t2t  is not None:
            if self.cost_style == 'sum':
                cost_t2t = cost_t2t.sum()
            else:
                cost_t2t=cost_t2t.mean()
...
class TripletLoss(nn.Module):
    ...
    def forward(self, sim_matrix_gt,sim_matrix_hardneg):

        # logpt = F.log_softmax(sim_matrix, dim=-1)
        score = torch.diag(sim_matrix_gt)
        hard_neg_sim = torch.diag(sim_matrix_hardneg)
        # nce_loss = -logpt

        cost_s = (self.margin + hard_neg_sim - score).clamp(min=0)
        sim_loss = cost_s.mean()
        return sim_loss
    

class HiTripletLoss(nn.Module):
    def __init__(self,margin=0):
        super(HiTripletLoss, self).__init__()
        self.margin = margin

    def forward(self, sim_matrix_gt,sim_matrix_hardneg):

        # logpt = F.log_softmax(sim_matrix, dim=-1)
        # score = torch.diag(sim_matrix_gt)## 不应该取出对角，应该是取出对角之外的coarse的句子的相似度再取平均
        
    
        upper_triangle = torch.triu(sim_matrix_gt, diagonal=1)
        lower_triangle = torch.tril(sim_matrix_gt, diagonal=-1)
        non_diagonal_elements = upper_triangle +lower_triangle 
       
        masked_non_diagonal_elements = non_diagonal_elements.masked_fill(non_diagonal_elements == 0, float('nan'))
        coarse_neg_sim = torch.nanmean(masked_non_diagonal_elements, dim=1)
        
        # 用所有negative的meanpooling 
        hard_neg_sim = torch.mean(sim_matrix_hardneg,dim=1)
        cost_s = (self.margin + coarse_neg_sim - hard_neg_sim ).clamp(min=0)
        sim_loss = cost_s.mean()
        return sim_loss
...
class CrossEn_maxcol(nn.Module):
    def __init__(self):
        super(CrossEn_maxcol, self).__init__()

    def forward(self, sim_matrix):
        # first_col = sim_matrix[:, 0]  # 获取第一列
        # logpt = F.log_softmax(first_col, dim=0)  # 对第一列进行softmax

        logpt = F.log_softmax(sim_matrix, dim=-1)#按最后一维进行softmax
        logpt = logpt[:,0]  #获取第一列
