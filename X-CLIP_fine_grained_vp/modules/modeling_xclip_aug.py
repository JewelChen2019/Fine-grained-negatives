...
class XCLIP(CLIP4ClipPreTrainedModel):
    ...
    # def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        if self.training:
            ...
            if self.loss_func == "maxcol_v2t_word" :
                loss = 0.
                ...
                loss += sim_loss_neg_word


            if self.loss_func == "maxcol_v2t_phrase" :
                loss = 0.
                ...
                loss += sim_loss_neg_phrase


            if self.loss_func == "maxcol_v2t_word_phrase" :
                loss = 0.
                v2negt_sim_matrix_word=[]
                for singel_video_feat,singel_video_mask,k_neg_sent_feat,k_neg_seq_features,k_neg_attention_mask in zip(visual_output,video_mask,neg_sequence_output,neg_seq_features,word_attention_mask_aug):
                    singel_video_feat =singel_video_feat.unsqueeze(dim=0)
                ...
                v2negt_sim_matrix_phrase = torch.cat(v2negt_sim_matrix_phrase, dim=0)
                sim_loss_neg_phrase = self.loss_fct_col(v2negt_sim_matrix_phrase)
                loss += sim_loss_neg_word + sim_loss_neg_phrase
            

            if self.loss_func == "maxcol_weight_coarse_word_phrase" :
                loss = 0.
                sim_matrix, *_tmp = self.get_similarity_logits(sequence_output, seq_features, visual_output, attention_mask, 
                                            video_mask, shaped=True, loose_type=self.loose_type)
                sim_loss1 = self.loss_fct(sim_matrix)
                ...
                v2negt_sim_matrix_phrase = torch.cat(v2negt_sim_matrix_phrase, dim=0)
                sim_loss_neg_phrase = self.loss_fct_col(v2negt_sim_matrix_phrase)
                sim_loss = self.coarse_weight*(sim_loss1 + sim_loss2 ) / 2 + self.word_weight *sim_loss_neg + self.phrase_weight*sim_loss_neg_phrase
                loss += sim_loss


            if self.loss_func == "maxcol_weight_word_phrase" :
                loss = 0.
                sim_matrix, *_tmp = self.get_similarity_logits(sequence_output, seq_features, visual_output, attention_mask, 
                                            video_mask, shaped=True, loose_type=self.loose_type)
                sim_loss1 = self.loss_fct(sim_matrix)
                ...
                v2negt_sim_matrix_phrase = torch.cat(v2negt_sim_matrix_phrase, dim=0)
                sim_loss_neg_phrase = self.loss_fct_col(v2negt_sim_matrix_phrase)
                sim_loss = (sim_loss1 + sim_loss2 ) / 2 + self.word_weight *sim_loss_neg + self.phrase_weight*sim_loss_neg_phrase
                loss += sim_loss
