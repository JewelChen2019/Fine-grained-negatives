...
def get_args(description='X-CLIP on hard negative samples augmentation Retrieval Task'):
    ...
    parser.add_argument('--vpt_lr', type=float, default=1., help='coefficient for vpt embedding.')
    ...
    parser.add_argument('--loss_func', type=str, default="fineGrained",
                        choices=["fineGrained", "triplet",'maxcol', "boundary", "maxcol_word",'maxcol_phrase',"hierachicalRank_word","hierachicalRank_phrase",'mix','maxcol_word_vpt','ft_maxcol_word_vpt','ft_maxcol_word_vpt_CLS','maxcol_phrase_vpt'],
...
def init_model(args, device, n_gpu, local_rank):
    ...
    if args.init_model:
        print("*"*20, "Load model from", args.init_model, "*"*20)
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    vpt_param_tp = [(n, p) for n, p in param_optimizer if "vpt_embedding" in n]
    decay_clip_param_tp = [(n, p) for n, p in decay_clip_param_tp if "vpt_embedding" not in n]
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in decay_clip_param_tp], 'weight_decay': weight_decay, 'lr': 1e-7},
        {'params': [p for n, p in decay_noclip_param_tp], 'weight_decay': weight_decay},
        {'params': [p for n, p in no_decay_clip_param_tp], 'weight_decay': 0.0, 'lr': 1e-7 },
        {'params': [p for n, p in no_decay_noclip_param_tp], 'weight_decay': 0.0},
        {'params': [p for n, p in vpt_param_tp], 'weight_decay': 0.0,'lr': args.vpt_lr }#decay_clip_param_tp
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for param_group in optimizer.param_groups:
        # print("Parameter Name:", param_group['params'])
    ...
    for step, batch in tqdm(enumerate(train_dataloader)):
        ...
        # loss = model(input_ids, segment_ids, input_mask, video, input_ids_aug, segment_ids_aug, input_mask_aug, video_mask)
        # print('----loss----', loss) 
        ###word and phrase
        ...
        loss= model(input_ids, segment_ids, input_mask, video, word_text_neg, word_segment_neg,word_mask_neg,phrase_text_neg,phrase_segment_neg,phrase_mask_neg, video_mask)
        ...
        # loss_v2t.appends(loss_v2t.item())
    ...
    # plt.plot(loss_t2v, label='loss_t2v', color='blue')
    # plt.plot(loss_v2t, label='loss_v2t', color='red')
    # plt.plot(loss_total, label='Loss ', color='green')
    # plt.plot(loss_word, label='loss_word', color='purple')
    # plt.plot(loss_phrase, label='loss_phrase', color='orange')
    # plt.xlabel('Epoch')
    # plt.ylabel('Loss')
